 ...
 * @Last Modified time: 2019-07-08 15:23:00
 */
import * as React from "react";
import { connect } from "react-redux";
import { Modal, Form, Icon, Input, Button, Checkbox, message, Row, Col } from "antd";
import { checkRegist, regist, login, getVerifyCode } from "../../actions/action";
...
class Login extends React.Component<Props, {}> {
  ...
  public handleSubmit = (e: React.FormEvent) => {
    ...
    form.validateFields(async (err: any, userInfo: LoginOrRegist) => {
      ...
      if (this.state.isLogin) {
        ...
        // 为注册时
        regist(userInfo).then((data: any) => message.success(data.content ? "注册成功" : "注册失败"));
      }
    });
  };
  /**
   * @function Login 登录
   */
  public checkRegist = async (rule: any, value: string, callback: any) => {
  ...
  public render() {
    ...
    return (
      <Modal title={this.state.isLogin ? "登录" : "注册"} visible={this.props.status} onCancel={this.cancelModal} footer={null}>
        <Form onSubmit={this.handleSubmit} className="login-form">
          ...
          <FormItem>
            ...
            Or
            <a href="javascript:;" onClick={() => this.setState({ isLogin: !this.state.isLogin })}>
...
const mapDispatchToProps = (dispatch: Dispatch) => {
  return {
    ...
    showLogin: (currentStatus: boolean) => dispatch(showLogin(currentStatus))
  };
};
